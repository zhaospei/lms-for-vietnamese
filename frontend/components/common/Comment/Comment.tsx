...
export default function Comment({
    ...
    return (
        <div className="flex flex-col">
            <TitleWithBox title={'Câu trả lời'} boxContent={cnt} size="middle" boxClassName="px-2 text-white" color="#AAD576"/>
            {/* <button onClick={() => setNew(4)}>Click me</button> */}
            <div style={{ marginTop: '40px', padding: '4px' }} >
                {/* <Editor />  */}
                <div className="editor__label"> Để lại một câu trả lời?
                  </div>
                <div className="answer_main">
                    ...
                    <div className="answer__details">
                        {/* <div className="answer__message"> */}
                            <textarea rows={4} className="mainText" name="answer" placeholder="Hãy để lại một câu trả lời (Lưu ý không spam, trả lời nhảm, thảo luận chính trị, công kích cá nhân/tập thể, ..." value={inputReply}  onChange={e => {setReply(e.target.value); }} >
                            </textarea>
                        {/* </div> */}
                        <div className="answer__btns">
                            <button className={inputReply===""?"button button-small answer__button disabled": "button button-small answer__button"} onClick={onSubmit}> Trả lời </button>
